...
run_kernel() {
    ...
    if [[ -z "$K_WORKDIR" ]]; then
        K_WORKDIR="$(mktemp -d /tmp/rodinia_bench_${KERNEL}_XXXXXX)"
        CLEANUP=1
    fi
    mkdir -p "$K_WORKDIR"
    ...
    # 1. Compile reference object file from Rodinia 3.1 reference source
...
if [[ "$TARGET" == "all" ]]; then
    ...
    PASSED=0
    echo "================================================================================"
    echo "Running Rodinia Benchmark Verification Suite (${#ALL_KERNELS[@]} kernels, ranks=$RANKS)"
    echo "================================================================================"
